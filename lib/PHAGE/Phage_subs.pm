...
our $VERSION = '2.6';
my $LOG; # declare lexical filehandle variable for the logfile
...
sub load_hash_from_file {
    ...
    my $descriptor = "";
    my $FILE;  # declare lexical filehandle variable
    for my $file (@{$fileref})  {
	print "LOAD_FILES:  filename = $file\n" if ($DEBUG);
        open ($FILE, '<::encoding(UTF-8)',"$path/$file") or &write_log("4","can't read file $file: $!\n");
	while (<$FILE>)  {
	    chomp;
...
sub get_HMM_size_trusted_noise  {
    ...
    my $list = "";
    my $i = "";
    my $IN; # declare lexical filehandle variable
    ...
    for ($i=0; $i<=$#files; $i++) {
	open ($IN, '<:encoding(UTF-8)', "$files[$i]") or &write_log("4","can't open file $files[$i]: $!\n");
	while (<$IN>)  {
	    chomp;
	    @a=split(/\t+/);
	    $key = $a[0];
...
sub get_ok_comnames {
    ...
    my @temp = ();
    my $name = "";
    my $COMFILE; # declare lexical filehandle variable
    open ($COMFILE, '<:encoding(UTF-8)', "$comfile") or &write_log("4","can't read file $comfile: $!\n");
    while (<$COMFILE>)  {
	chomp;
        @temp = split(/\s+/);
...
sub get_DB_info {
    ...
    my @temp = ();
    my $tag = "";
    my $DBFILE; # declare lexical filehandle variable
    open ($DBFILE, '<:encoding(UTF-8)', "$DBfile") or &write_log("4","can't read file $DBfile: $!\n");
    while (<$DBFILE>)  {
	chomp;
        @temp = split(/\t+/);
	$tag = $temp[0];
...
sub populate_genomearray {
  ...
  my $pos = 0;
  my $feat_name = "";
  foreach my $keys (sort {$a <=> $b} keys %{$reref->{$asmbl_id}}) {
...
sub create_log {
  ...
  if (-e "$logfile.log" == 1) {  # check for phage_phinder.log present
    ...
    open ($LOG, '>:encoding(UTF-8)', "$logfile\_$i.log") or &write_log("4","can't open file $logfile\_$i.log: $!\n");
    return ("$logfile\_$i.log");
  }
  else  {  # else, make phage_phinder.log for the first time
    open ($LOG, '>:encoding(UTF-8)', "$logfile.log") or &write_log("4","can't open file $logfile.log: $!\n");
    return ("$logfile.log");
  }
}
...
sub pttcoords_to_info { # convert Genbank .ptt (.coords) file to phage_finder_info.txt format
    ...
    my $accession = "";
    my $asmbl_id = "";
    my ($GENOME,$INFILE,$OUTFILE); # declare lexical filehandle variables
...
# do not store any data #
	...
	open ($GENOME, '<:encoding(UTF-8)', "$seqfile") or &write_log("4","can't find the sequence file $seqfile:$!\n");
	...
	$size = pop(@length);
        ...
        open ($INFILE, '<:encoding(UTF-8)', "$pttfile") or &write_log("4","can't open the ptt file $pttfile: $!\n");
    ...
    open ($OUTFILE, '>:encoding(UTF-8)', "$infofile") or &write_log("4","can't create file $infofile: $!\n");
...
sub check_infofile {
    ...
    my @a = ();
    my $rvalue = "";
    my $INFILE; # declare lexical filehandle variable
    open ($INFILE, '<:encoding(UTF-8)', "$infofile") or &write_log("4","can't open infofile $infofile: $!\n");
    chomp;
    @a = split(/\s+/, <INFILE>);
    if ($a[1] =~ /^\D/) {
        print "$infofile\n" if ($DEBUG);
...
sub get_gene_info {
  ...
  my $count = "";
  my $name = "";
  my $INFOFILE; # declare lexical filehandle variable
  ...
  open ($INFOFILE, '<:encoding(UTF-8)', "$infofile") or &write_log("4","can't open file $infofile: $!\n");
  while (<$INFOFILE>)  {
      chomp;
...
sub get_tRNAs  {
    ...
    my $featname = "";
    my $type = "";
    my $TRNA; # declare lexical filehandle variable

      open ($TRNA, '<:encoding(UTF-8)', "$tRNA_file") or &write_log("4","can't read file $tRNA_file: $!\n");
      while($line = <$TRNA>) {
        chomp($line);
	...
	if ($a[0] =~ /.*\|(NC_\d+)/) { # get NC refseq number 
      ...
      close ($TRNA);
      foreach my $key (sort {$a <=> $b} keys %{$tRNAref}) { # sort by asmbl_id
        foreach my $yek (sort {$a <=> $b} keys %{$tRNAref->{$key}}) { # then sort the tRNAs by end 5
...
sub get_tmRNAs  {
    ...
    my $featname = "";
    my $hold = "";
    my $TMRNA; # declare lexical filehandle variable

      open ($TMRNA, '<:encoding(UTF-8)', "$tmRNA_file") or &write_log("4","can't read file $tmRNA_file: $!\n");
      while($line = <$TMRNA>) {
        chomp($line);
...
sub get_ncRNAs  { # obtain data from the output of cmscan (ncRNA_cmscan.out)
    ...
    my $featname = "";
    my $comname = "";
    my $NCRNA; # declare lexical filehandle variable
    ...
    open ($NCRNA, '<:encoding(UTF-8)', "$ncRNA_file") or &write_log("4","can't read file $ncRNA_file: $!\n");
    while($line = <$NCRNA>) {
	chomp($line);
...
sub select_featnames_from_btab { # populate a hash (index) with featnames (key) and 1 as value
    ...
    open ($INFILE, '<:encoding(UTF-8)', "$btabfile") or &write_log("4","can't open file $btabfile: $!\n");
    chomp;
    @a = split(/\t+/, <$INFILE>);
    if ($a[3] =~ /washu/) {
...
# ========================================================
	...
	foreach my $descriptor (keys %{$lists_ref})  {
...
sub find_hmms {  # find HMM hits to bacteriophage proteins
    ...
    my $scores = 0;
    my $HMMFILE; # declare lexical filehandle variable
    &get_HMM_size_trusted_noise($phome,$HMMversion,\%HMM_master_hash); # get the size, trusted and noise cut-off values

    for my $file (@{$data}) { # loop through list of filenames, either user-specified (GLOCAL) or default (both GLOCAL and FRAG)
	if (-s "$file") { # changed 03/07/14 to simplify code dfouts
	    ...
	    open ($HMMFILE, '<:encoding(UTF-8)', "$file") or &write_log("4","can't open file HMM datafile $file: $!\n");
	    while (<$HMMFILE>)  {
		if ((/^Query HMM:\s+(\S+)/)
                   || (/^Query:\s+(\S+)/)) {
		    $query_HMM = $1;
			...
			if ((($total_score > $noise) && ($model eq "GLOCAL")) || (($total_score > $trusted) && ($model eq "FRAG")))  {  # if score is not already recorded and is above the noise cut-off for GLCAL model or trusted for FRAG model
			    ...
			    foreach my $descriptor (keys %{$HMMref})  {
...
sub get_assemblies {
  ...
  my $record;
  my $status;
  my $GENOME; # declare lexical filehandle variable

  #Open and read file with fasta format sequences.
  open ($GENOME, '<:encoding(UTF-8)', "$file") or &write_log("4","can't find the file $file: $!\n");
  my ($save_input_separator) = $/;
  $/="\n>";
  while (<$GENOME>) {
...
sub get_TSD {
    ...
    my $int2use = "";
    my $diffRI = "";
    my $makedb = ""; # stores the location and name of makeblastdb
    my $blastn = ""; # stores the location and name of blastn
    my $mummer = ""; # stores the location and name of mummer
    my $fasta = "";  # stores the location and name of fasta
    my ($HEADFILE,$TAILFILE,$MUM,$FASTA,$BLAST,$ATTFILE); # declare lexical filehandle variables
    
    if ($type =~ /phage/)  {
	print "=======> $type <=======\n" if ($DEBUG);
	open ($HEADFILE, '>:encoding(UTF-8)', "$head_filename") or &write_log("4","1 can\'t open file $head_filename: $!\n");
	open ($TAILFILE, '>:encoding(UTF-8)', "$tail_filename") or &write_log("4","2 can\'t open file $tail_filename: $!\n");
    }
...
########### Do searches based on $method ############
    ...
    if ($method eq "mummer")  {
	if ($cfg->val('search','mummer')){
	    $mummer = $cfg->val('search','mummer');
	}
	else {
	    die ("FATAL error: The config.ini file did not have a value \"mummer\"\n");
	}
	system ("$mummer -mum -l 5 $tail_filename $head_filename | sort -nrk3 | head -2 > $data_prefix\_mummer.out");
	open ($MUM, '<:encoding(UTF-8)', "$data_prefix\_mummer.out") or &write_log("4","3 can\'t open file $data_prefix\_mummer.out: $!\n");
	$n = 1;
	while (<$MUM>) {
	    chomp;
	    @a = split(/\s+/);
	...
	if ($cfg->val('search','fasta')){
	    $fasta = $cfg->val('search','fasta');
	}
	else {
	    die ("FATAL error: The config.ini file did not have a value \"fasta\"\n");
	}
        system ("$fasta -H -m 9 -w 60 -O $data_prefix\_fasta.out -n -b 4 -q -d 4 -r +4/-5 -3 $head_filename $tail_filename > /dev/null");
        open ($FASTA, '<:encoding(UTF-8)', "$data_prefix\_fasta.out") or &write_log("4","3 can\'t open file $data_prefix\_fasta.out: $!\n");
        $n = 1;    
        # =================================================================================================================
	...
	if ($cfg->val('search','makeblastdb')){
	    $makedb = $cfg->val('search','makeblastdb');
	}
	else {
	    die ("FATAL error: The config.ini file did not have a value \"makeblastdb\"\n");
	}
        system ("$makedb -in $tail_filename -dbtype nucl > /dev/null 2>&1"); # make the tail sequence BLAST searchable
	if ($cfg->val('search','blastn')){
	    $blastn = $cfg->val('search','blastn');
	}
	else {
	    die ("FATAL error: The config.ini file did not have a value \"blastn\"\n");
	}
	system ("$blastn -task blastn -db $tail_filename -query $head_filename -out $data_prefix\_blast.out -outfmt 6 -strand plus -max_target_seqs 4 -num_threads 2 -dust no > /dev/null 2>&1");
        open ($BLAST, '<:encoding(UTF-8)', "$data_prefix\_blast.out") or &write_log("4","4 can\'t open file $data_prefix\_blast.out: $!\n");
        $n = 1;
        # ========================================================
...
#################### CHOOSE ATT SITE ##################
	...
	if (($temp_region_size >= $minsize) && ($temp_region_size >= 6000)){ # if the region defined by putative att sites at least 9 kbp originally (to elminate short junk regions with tRNA homology), but changed to 6kb on 10/24/2013 for Acinetobacter non-strict region on a plasmid (not phage, but may be interesting)
	    ...
	    $attR = $att_hash{$best}->{'attR'}; # relative to genome
            ...
            open ($ATTFILE, '>:encoding(UTF-8)', "$data_prefix\_att.out") or &write_log("4","can\'t open file $data_prefix\_att.out: $!\n");
...
LOOP:
  foreach my $key (sort {$a <=> $b} keys %{$phageref}) { #added sorting of the regions to make sequencial for renumbering when a regions is deleted
      ...
      %core_cnts = (); # clear out the hash counter for each phage region! added 07/31/24 by dfouts
	...
	if (exists $hitref->{$feat_name}->{'Large Terminase'}) {
	  ...
	  if ($core_cnts{'Large Terminase'} > 1) { # if more than one Large Terminase gene in this region, flag as a piggyback
	      ...
	      print "==> piggyback - $core_cnts{'Large Terminase'} Large Terminase genes detected\n" if ($DEBUG);
	  }
	}
	  ...
	  if ($core_cnts{'Portal'} > 1) { # if more than one Portal gene in this region, flag as a piggyback
	      $phageref->{$key}->{'piggyback'} = 1;
	      print "==> piggyback - $core_cnts{'Portal'} Portal genes detected\n" if ($DEBUG);
	  }
	}
	  ...
	  if ($core_cnts{'Major Capsid'} > 1) { # if more than one Major Capsid gene in this region, flag as a piggyback
	      $phageref->{$key}->{'piggyback'} = 1;
	      print "==> piggyback - $core_cnts{'Major Capsid'} Major Capsid genes detected\n" if ($DEBUG);
	  }
	}
	...
	foreach my $hmm (keys %{$hitref->{$feat_name}->{'hmm'}}) {
	  ...
	  if (($HMMref->{'Core HMM'}{$hmm} == 1) && ($score >= $trusted)){ # core hit must be >= trusted (getting some real junk hits in oral microbes)
		...
		$phageref->{$key}->{'piggyback'} = 1;
		print "==> piggyback - $core_cnts{'hmm'} $hmm core HMM matches detected\n" if ($DEBUG);
	    }
	  }
