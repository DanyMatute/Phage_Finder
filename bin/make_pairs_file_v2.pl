...
use strict;
use Getopt::Std;
getopts ('b:de:f:g:hi:l:p:rV');

############## Declare global variables #################
our ($opt_h,$opt_b,$opt_d,$opt_e,$opt_f,$opt_g,$opt_i,$opt_l,$opt_p,$opt_r,$opt_V);
my ($btab_file, $frag_file, $gene_att, $evalue, $percentid, $min_hit_length, $order_by_ref, $pep_file, $DEBUG);
my $version = "2.0";

############## parse options ###############
if ($opt_h) { &option_help; }
if ($opt_V) {die "$version\n";}
if ($opt_r) {$order_by_ref = 1;}
if (-s $opt_b) {
    $btab_file = $opt_b;
}
else {
...
sub option_help {
  ...
  Option:
    ...
    -r: relative to reference (e.g., first line of frag.file) [DEFAULT = 0]
...
if ($order_by_ref) {
    print "order by reference output chosen...\n" if ($DEBUG);
    for ($i=0; $i<($#order); $i++){
	$j = $i+1;
	print "$i = $order[$i]\n" if ($DEBUG);
	foreach $key1 (sort {$a <=> $b} keys %{$Locus_lookup{$order[0]}}) { # not advancing the order so that all hits are relative to the first line of the frag.file
	    #print "\t$key1\n" if ($DEBUG);
	    foreach $key2 (keys %{$Locus_lookup{$order[$j]}}) {
		#print "\t\t$key2\n" if ($DEBUG);
		if ($relationship_hash{$key1}{$key2}->{'id'}) {
		    print OUTFILE "$order[0]\t$key1\t$order[$j]\t$key2\t$relationship_hash{$key1}{$key2}->{'id'}\n"; # again, the order for the first genome is held at array position 0
		}
	    }
	}
    }
    ...
    print "default output order chosen...\n" if ($DEBUG);
    for ($i=0; $i<($#order); $i++){
	$j = $i+1;
	print "$i = $order[$i]\n" if ($DEBUG);
