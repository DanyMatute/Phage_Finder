...
# ver 2.6 added the use of phage_finder.ini file to specify the location of dependency software like blast, makeblastdb, etc.  Removed variable blastbin (not needed)
...
use Config::IniFiles; # added in ver 2.6 dfouts 07/25/24
...
use Getopt::Std;
getopts ('b:c:df:hi:m:n:p:r:s:t:w:A:C:E:F:H:P:SV'); # Rearranged alphabetically per Phil Goetz's suggestion 04/16/13 (except I separate lc from uc)

our ($opt_b,$opt_c,$opt_d,$opt_f,$opt_h,$opt_i,$opt_m,$opt_n,$opt_p,$opt_r,$opt_s,$opt_t,$opt_w,$opt_A,$opt_C,$opt_E,$opt_F,$opt_H,$opt_S,$opt_V);
my ($asmbl_id,$basedir,$cfg,$config_file,$mode_basedir,$logfile,$default_hpw,$window,$step,$evalue,$btabpath,$btabfile,$infofile,$hmmdir,$hmmfile,$tRNA_data,$tmRNA_data,$ncRNA_data,$asmbly_file_path,$asmbly_file,$search_method,$strict,$mode,$badcomnamesfile,$frag_line_num,$DEBUG);
my ($APISFILE,$ATTFILE,$BCNFILE,$CAPSIDFILE,$CONFILE,$FRAGFILE,$LARGETERMFILE,$PEPFILE,$PORTALFILE,$PROTEASEFILE,$RBPSFILE,$SCAFFOLDFILE,$SEQFILE,$SERINERECOMBFILE,$SMALLTERMFILE,$TAB,$TOXINSFILE,$TYROSINERECOMBFILE); # declare lexical filehandle variables
my $version = "2.6";
...
elsif (($opt_F eq "F") || ($opt_F eq "f"))  {$search_method = "fasta";} # changed from fasta33 in ver 2.6
elsif (($opt_F eq "B") || ($opt_F eq "b")) {
    $search_method = "blastn"; #changed from "blast" in ver 2.6
}
else {
    $search_method = "blastn";
}

if (($opt_C) && (-s "$basedir/$opt_C")) { # if a config.ini file is provided, check that it is there and has size else check for the default phage_finder.ini file
    $config_file = $opt_C;
    print "number one\n";
}
elsif (-s "$Bin/../phage_finder.ini") { # no config.ini was specified so check for the default phage_finder.ini in the bin dir
    $config_file = "$Bin/../phage_finder.ini";
}
else {
    die "FATAL error: Could not find the config.ini file $config_file";
}
...
sub find_regions {
  ...
  foreach my $fig (sort {$a <=> $b} keys %{$figref}) {
...
sub find_5prime_end {
  ...
  for ($i = $#{$aref}; $i >= 0; $i--)  {
    ...
    if ($n == "0") {
      ...
      my $lmw = nlowmult($window, $aref->[$i])-$window;
      my $lms = nlowmult($step, $aref->[$i])-$step; #added 08/01/24 by dfouts
      print "find_5prime_end: n = $n\tworking on $reref->{$asmbl_id}{$aref->[$i]}->{'featname'}, clean_name = <$hitref->{$reref->{$asmbl_id}{$aref->[$i]}->{'featname'}}->{'clean_name'}>, lowmultwindow = \[$lmw\], lowmultstep = \[$lms\]...\n" if ($DEBUG);
      ...
      print "nextWINDOWcounts: $figref->{$lmw}->{'counts'}\tNAME: $figref->{$lmw}->{'featnames'}[0]..$figref->{$lmw}->{'featnames'}[$#{ $figref->{$lmw}->{'featnames'} }]\n" if ($DEBUG);
	     ...
	     ($hitref->{$figref->{nlowmult($window, $aref->[$i])}->{'featnames'}[0]}->{'end5'} < $aref->[$i]))
	{
      ...
      ### special note:  changed counts from >= 2 to >= 1 for the Campylobacter jejuni 84-25 genome since there were 2 peaks separated with a single hit in one window
	     ...
	     ($figref->{nlowmult($step, $aref->[$i])-$step}->{'counts'} >= 1) && 
	     ($hitref->{$figref->{nlowmult($step, $aref->[$i])-$step}->{'featnames'}[0]}->{'end5'} < $aref->[$i]))
	{
	  $phageref->{$n}->{'memberhash'}{$aref->[$i]}->{'featname'} = $reref->{$asmbl_id}{$aref->[$i]}->{'featname'};
	  $lastkey = $aref->[$i];
	  $phageref->{$n}->{'last5ORF'} = $lastkey;
      ...
      # case where >= 2 hits (changed from 3) in next window, but no clean name (bad annotation perhaps or something novel) and the ORF is upstream (5') of the current ORF
      # added lms and lmw in cases where the left most ORF in back to back steps is the same. 

      elsif ((($figref->{$lms}->{'counts'} >= 2) || ($figref->{$lmw}->{'counts'} >= 2)) && 
	     (($hitref->{$figref->{$lms}->{'featnames'}[0]}->{'end5'} < $aref->[$i]) ||
	      ($hitref->{$figref->{$lmw}->{'featnames'}[0]}->{'end5'} < $aref->[$i])))
	{
	  $phageref->{$n}->{'memberhash'}{$aref->[$i]}->{'featname'} = $reref->{$asmbl_id}{$aref->[$i]}->{'featname'};
	  $lastkey = $aref->[$i];
	  $phageref->{$n}->{'last5ORF'} = $lastkey;
      ...
      #if there are at least 2 hits in the current step and end5 of first featname having hit in current window is upstream  of current orf, include
...
sub find_3prime_end { # relative to contig 5' --->
  ...
  for my $i ( 0 .. $#{$aref} )  {
	...
	until ((($end5-$lastkey) > 10000) || ($pos > $#{$aref})){ # only check within 10 kb of lastkey
	  ...
	  elsif (($hitref->{$reref->{$asmbl_id}{$end5}->{'featname'}}->{'hit'} == 1) || (exists $hitref->{$reref->{$asmbl_id}{$end5}->{'featname'}}->{'hmm'})) { ### added 07/30/2024 by dfouts in the case when the phage region extends past the current window and the next window has too few hits (edge effect). We will need to check if the current ORF has a phage_db hit or phage HMM hit
	    $clean = 1;
	    print "find_3prime_end_clean:  [phage_db or HMM hit] clean = <$clean>, pos = <$pos>, end5 = <$end5>, locus = <$reref->{$asmbl_id}{$end5}->{'featname'}>, annotation = <$hitref->{$reref->{$asmbl_id}{$end5}->{'featname'}}->{'com_name'}>\n" if ($DEBUG);
	  }
	  elsif ((exists $tRNAref->{$asmbl_id}{$end5}) || ($okref->{$hitref->{$reref->{$asmbl_id}{$end5}->{'featname'}}->{'clean_name'}} == 1)) { # added check for tRNA/tmRNA 07/30/24 by dfouts
	    $clean = 1;
	    print "find_3prime_end_clean:  [tRNA/tmRNA or clean com_name] clean = <$clean>, pos = <$pos>, end5 = <$end5>, locus = <$reref->{$asmbl_id}{$end5}->{'featname'}>, annotation = <$hitref->{$reref->{$asmbl_id}{$end5}->{'featname'}}->{'com_name'}>\n" if ($DEBUG);
	  }
	  else { # need to add to the list of bad com_names
	    $clean = ""; # found a bad com_name, so bonk it
	    print "find_3prime_end_not_clean:  [ bad com_name ] clean = <$clean>, pos = <$pos>, end5 = <$end5>, locus = <$reref->{$asmbl_id}{$end5}->{'featname'}>, annotation = <$hitref->{$reref->{$asmbl_id}{$end5}->{'featname'}}->{'com_name'}>\n" if ($DEBUG);
	  }
	  $pos++;
	  $end5 = $aref->[$pos];
	}
...
###########################
	...
	my ($key,$Bint,$graseref) = @_;  # passed the $key variable because the local function was unable to see it otherwise
	...
	($found_att, $hold_attB, $hold_attE) = &get_TSD($cfg,$prefix,$asmbl_id,$hitref,$phageref,$reref,$aref,$DEBUG,"+","phage",$search_method,$key,$ahref,$start_head,$start_tail,$size_head,$size_tail,"0","0",$figref,$window,$step,$hitsperwindow,$halfsize,$Bint,$graseref);
...
############################
	...
	my ($key,$Eint,$graseref) = @_; # passed the $key variable because the local function was unable to see it otherwise
	$extension = $Bextension;
	...
	($found_att, $hold_attB, $hold_attE) = &get_TSD($cfg,$prefix,$asmbl_id,$hitref,$phageref,$reref,$aref,$DEBUG,"-","phage",$search_method,$key,$ahref,$start_head,$start_tail,$size_head,$size_tail,"0","0",$figref,$window,$step,$hitsperwindow,$halfsize,$Eint,$graseref);
	### could move to get_TSD ###
	if ($found_att)  { # calculate the distance from outermost integrase coord to outermost att coord
...
############################ Find tRNA att sites ##############################################################################
	...
	my ($key,$Bint,$Eint,$graseref) = @_; # passed the $key variable because the local function was unable to see it otherwise
        $extension = $Bextension;
	$fivetend5 = $tRNA5;
	...
	if (defined $fivetend5)  { # if tRNA is on the 5' side of the region
		...
		($found_att, $hold_attB, $hold_attE) = &get_TSD($cfg,$prefix,$asmbl_id,$hitref,$phageref,$reref,$aref,$DEBUG,$direction,"phage\_$reref->{$asmbl_id}{$tRNA5}->{'featname'}",$search_method,$key,$ahref,$start_head,$start_tail,$size_head,$size_tail,"0","0",$figref,$window,$step,$hitsperwindow,$halfsize,$int,$graseref);
		...
		if ($found_att == 1)  { # if match tRNA sequence, then check to see if we can expand on the match on 3' side
		    ...
		    print "find_att_sites:tRNA_sub: start_tail = $start_tail\n" if ($DEBUG);
		    print "find_att_sites:tRNA_sub: size_tail = $size_tail\n" if ($DEBUG);
		    &get_TSD($cfg,$prefix,$asmbl_id,$hitref,$phageref,$reref,$aref,$DEBUG,$direction,"phage\_$reref->{$asmbl_id}{$tRNA5}->{'featname'}\_r2",$search_method,$key,$ahref,$start_head,$start_tail,$size_head,$size_tail,$hold_attB,$hold_attE,$figref,$window,$step,$hitsperwindow,$halfsize,$int,$graseref);
	...
	if ((defined $threetend5) && ($found_att == 0)) { # if tRNA is on the 3' side of the region
		...
		print "find_att_sites:tRNA_sub: start_tail = $start_tail\n" if ($DEBUG);
		print "find_att_sites:tRNA_sub: size_tail = $size_tail\n" if ($DEBUG);
		($found_att, $hold_attB, $hold_attE) = &get_TSD($cfg,$prefix,$asmbl_id,$hitref,$phageref,$reref,$aref,$DEBUG,$direction,"phage\_$reref->{$asmbl_id}{$tRNA3}->{'featname'}",$search_method,$key,$ahref,$start_head,$start_tail,$size_head,$size_tail,"0","0",$figref,$window,$step,$hitsperwindow,$halfsize,$int,$graseref);
		if ($found_att == 1)  { # if match tRNA sequence, then check to see if we can expand on the match on 3' side
		    print "find_att_sites:tRNA_sub: second round expanded tRNA search...\n" if ($DEBUG);
		    ...
		    print "find_att_sites:tRNA_sub: start_tail = $start_tail\n" if ($DEBUG);
		    print "find_att_sites:tRNA_sub: size_tail = $size_tail\n" if ($DEBUG);
		    &get_TSD($cfg,$prefix,$asmbl_id,$hitref,$phageref,$reref,$aref,$DEBUG,$direction,"phage\_$reref->{$asmbl_id}{$tRNA3}->{'featname'}\_r2",$search_method,$key,$ahref,$start_head,$start_tail,$size_head,$size_tail,$hold_attB,$hold_attE,$figref,$window,$step,$hitsperwindow,$halfsize,$int,$graseref);
...
############################ End tRNA att-finding ##################################################################################
    foreach my $key (sort {$a <=> $b} keys %{$phageref}) {
...
# new logic to find 5' and 3' integrases
	...
	if ((exists $phageref->{$key}->{'tRNA'}) && ($int == 1))  { # tRNA is present and an integrase in the region, find tRNA TSD
		...
		foreach my $t (sort {$a <=> $b} keys %{$phageref->{$key}->{'tRNA'}}) { # get most 5' tRNA
		...
		foreach my $t (sort {$b <=> $a} keys %{$phageref->{$key}->{'tRNA'}}) { # get most 3' tRNA
		    print "t: $t\ttRNA: $phageref->{$key}->{'tRNA'}{$t}\n" if ($DEBUG);
...
sub print_regions {
    ...
    my ($OUTsmall,$OUTmedium,$OUTPUT); # filehandle lexical variable
	...
	print STDOUT "PRINT_SUB: DIRECTION: <$phage_direction>\n" if ($DEBUG);
	print STDOUT "PRINT_SUB: contig_end5 = <$contig_end5>, contig_end3 = <$contig_end3>\n" if ($DEBUG);
	print STDOUT "PRINT_SUB: phage_end5 = <$phage_end5>, phage_end3 = <$phage_end3>\n" if ($DEBUG);

	## populate the $Best_hits has so that we can determine the best database hit for this phage region
	
	foreach my $k (sort {$a <=> $b} keys %{$phageref->{$key}->{'memberhash'}}) { # pre loop to get the Best Hit
	    $featname = $phageref->{$key}->{'memberhash'}{$k}->{'featname'};
	    print STDOUT "PRINT_SUB: BEST_HITS: pre_featname = $featname\n" if ($DEBUG);
	    if ((exists $hitref->{$featname}->{'annotation'}) && ($featname !~ /^att/) && ($featname !~ /^tm?RNA/) && ($featname !~ /^ncRNA/)){
		print STDOUT "PRINT_SUB: ===> Region $key contig_coords: $contig_end5 -> $contig_end3 (phage_coords: $phage_end5 to $phage_end3), annotation = $hitref->{$featname}->{'annotation'}, end5 = $hitref->{$featname}->{'end5'}\n" if ($DEBUG);
		if (($hitref->{$featname}->{'end5'} >= $contig_end5) && ($hitref->{$featname}->{'end5'} <= $contig_end3)) {
		    $Best_hits{$hitref->{$featname}->{'phage'}}->{'counts'}++;
		    $totalhits++;
		    print STDOUT "PRINT_SUB: BEST_HITS:    post_featname: $featname\t$hitref->{$featname}->{'annotation'}\t$hitref->{$featname}->{'phage'}\n" if ($DEBUG);
		}
	    }
	}
	...
	foreach my $hit (sort {$Best_hits{$b}->{'counts'} <=> $Best_hits{$a}->{'counts'}} keys %Best_hits)  {
	    $best = $hit;
	    $shortag = $best;
	    $shortag =~ s/^PHAGE_//;
	    last;
	...
	if ($size_att > 1) { # att found
	    print STDOUT "PRINT_SUB: print_regions: att found ...\n" if ($DEBUG);
		...
		print STDOUT "PRINT_SUB: print_regions: target detected...\n" if ($DEBUG);
		$target = $phageref->{$key}->{'target'};
	    }
	    else { # no target was detected
		print STDOUT "print_regions: no target detected but att found ...\n" if ($DEBUG);
		$target = "N.D.";
	    }
	}
	else { # no att found
	...
	print STDOUT "$label $type region $region_id is from $phage_end5 to $phage_end3 and is $region_size bp in size, gc%: $phageref->{$key}->{'gc'}%, TARGET: $target, Piggback: $is_piggy. Best hit: $best ($Best_hits{$best}->{'counts'}/$totalhits)\n";
	...
	foreach my $yek (sort {$a <=> $b} keys %{$phageref->{$key}->{'memberhash'}}) {
	    $featname = $phageref->{$key}->{'memberhash'}{$yek}->{'featname'};
	    print STDOUT "PRINTSUB_memberhash_loop: $phage_end5 [ $yek ] $phage_end3, featname = <$featname>\n" if ($DEBUG);
		...
		if (exists $hitref->{$featname}->{'annotation'}) {
			...
			print STDOUT "printatt($RNA_type): featname = <$featname>, com_feat = <$com_feat>, end5 = <$end5>, end3 = <$end3>\n" if ($DEBUG);
		...
		if ($print5 == 0) { # print first ORF in the region
		    ...
		    print STDOUT "=>5prime ORF end5 = $key ($rehash{$asmbl_id}{$key}->{'featname'}), 5flank ORF end5 = $5flank ($rehash{$asmbl_id}{$flank5}->{'featname'})\n" if ($DEBUG);
		    ###
		}
		...
		if (exists $hitref->{$featname}->{'hmm'}) {
		    foreach my $HMM_name (sort {$a <=> $b} keys %{$hitref->{$featname}->{'hmm'}}) {
	...
	print STDOUT "PRINTSUB: flank5 = <$flank5>, contig_end5 = <$contig_end5>\n" if ($DEBUG);
        print STDOUT "PRINTSUB: flank3 = <$flank3>, contig_end3 = <$contig_end3>\n" if ($DEBUG);		 
	...
	print STDOUT "=>3prime ORF end5 = $key ($flank5_gene), 3flank ORF end5 = $flank3 ($flank3_gene), contig_size = <$asmbl_idhash{$asmbl_id}->{'genomesize'}>\n" if ($DEBUG);
    ...
    if (($max > 1) || ($max == 0))  {
	print STDOUT "There are $max putative large/medium/small phages in $asmbl_id!\n";
	print STDOUT "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n";
    }
    elsif ($max == 1)  {
	print STDOUT "There is $max putative large/medium/small phage in $asmbl_id!\n";
	print STDOUT "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n";
    }
...
$end5,   $printfeatname,         $annotation
.
	foreach my $key (sort {$a <=> $b} keys %{$phageref}) { #may want to make this a for loop because some keys are being renumbered on the fly, causing some to not get examined and printed when I actually delete and rekey
	    $label = "";
	    $print_files = undef;
	    $size = abs($phageref->{$key}->{'5prime'} - $phageref->{$key}->{'3prime'})+1;
	    $size_att = abs($phageref->{$key}->{'5prime_att'} - $phageref->{$key}->{'3prime_att'})+1;
	    print STDOUT "print_regions: phage $key, size_att = $size_att\n" if ($DEBUG);
		...
		print STDOUT "Print_regions Main loop: phage direction = <$phage_direction>\n" if ($DEBUG);
	    }
		...
		print STDOUT "Print_regions Main loop: phage direction set to <$phage_direction>\n" if ($DEBUG);
	    }
		...
		if ($phage_direction eq "+") { # phage is -----> (brought this code back so phage will be in correct ori if int on one side, but contig end on other side)
		    ...
		    print STDOUT "Print_regions+:  >NO ATT< phage_end5 = ($phage_end5), phage_end3 = ($phage_end3), direction = ($phage_direction)\n" if ($DEBUG);
		}
	    ...
	    #
	    if ($phageref->{$key}->{'piggyback'} > 0) { #set variable for pretty printing of is_piggyback
		$is_piggy = "Y";}
	    else {
		$is_piggy = "N";
	    }
		...
		if ($small_open == 0)  { # only open each file if needed and only once per invocation of the script so we can get multiple phages per file
		    open ($OUTsmall, '>', "$write_dir/$asmbl_id\_$hitsperwindow\_phpico\_$window\_$step\_$evalue.out") or die "can't open file $write_dir/$asmbl_id\_$hitsperwindow\_phpico\_$window\_$step\_$evalue.out\n";
		...
		if (($strict == 0) || (($strict == 1) && (($phageref->{$key}->{'type'} eq "prophage") || ($phageref->{$key}->{'type'} eq "bacteriocin"))))  {
		    if ($med_open == 0)  {
			open ($OUTmedium, '>', "$write_dir/$asmbl_id\_$hitsperwindow\_phmedio\_$window\_$step\_$evalue.out") or die "can't open file $write_dir/$asmbl_id\_$hitsperwindow\_phmedio\_$window\_$step\_$evalue.out\n";
			{ # set per-filehandle variables
			    select( $OUTmedium );
			    $~ = 'OUTmedium';
			    $^ = 'OUTmedium_TOP';
		    ...
		    print STDOUT "L1: Region $key (type $phageref->{$key}->{'type'}) is unlikely a prophage, deleting ...\n" if ($DEBUG);
		...
		if (($strict == 0) || (($strict == 1) && (($phageref->{$key}->{'type'} eq "prophage") || ($phageref->{$key}->{'type'} eq "bacteriocin"))))  {
		    if ($large_open == 0)  {
			open ($OUTPUT, '>', "$write_dir/$asmbl_id\_$hitsperwindow\_phregions\_$window\_$step\_$evalue.out") or die "can't open file $write_dir/$asmbl_id\_$hitsperwindow\_phregions\_$window\_$step\_$evalue.out\n";
			{ # set per-filehandle variables
			    select( $OUTPUT );
			    $~ = 'OUTPUT';
			    $^ = 'OUTPUT_TOP';
		    ...
		    print STDOUT "L2: Region $key (type $phageref->{$key}->{'type'}) is unlikely a prophage, deleting ...\n" if ($DEBUG);
		    $phage_regions--; # deprecate phage_region counter
		}
	    }
	    else {
		print STDOUT "L3: Region $key is unlikely a prophage, deleting ...\n" if ($DEBUG);
		$phage_regions--;  # deprecate phage_region counter
	    }
	...
	printf STDOUT "There are $large regions > 18 Kb and summing $totalphagebp bp of sequence (%5.2f", $percent;
	print STDOUT "% of the genome)\n";
	
	printf STDOUT "There are $medium regions between 10 and 18 Kb and summing $totalmediumbp bp of sequence (%5.2f", $percentmedium;
	print STDOUT "% of the genome)\n";
	
	printf STDOUT "There are $small regions < 10 Kb and summing $totalsmallbp bp of sequence (%5.2f", $percentsmall;
	print STDOUT "% of the genome)\n";
...
sub write_output {  # write xgraph file and report file
    ...
    my $FH; # variable to hold the xgraph output lexical filehandle
    open ($FH, '>:encoding(UTF-8)', "$write_dir/$asmbl_id\_$hitsperwindow\_phgraph\_$window\_$step\_$evalue.out") || die "can't open file phgraph$window.out.\n";
    print $FH "title = $title id $asmbl_id (>= $minimum_hits hits per window)\n";
    print $FH "title_x = Position (bp)\n";
    print $FH "title_y = # Hits per $window bp window/$step bp step\n";
    print $FH "color = red\n";
    print $FH "thickness = 1\n";
    my $REPORT; # variable to hold the phreport lexical filehandle
    open ($REPORT, '>:encoding(UTF-8)', "$write_dir/$asmbl_id\_$hitsperwindow\_phreport\_$window\_$step\_$evalue.out") || die "can't open file phreport$window.out.\n";
    foreach my $fig (sort {$a <=> $b} keys %{$figref}) {
	...
	if (defined($figref->{$fig}->{'peak_number'})) { # changed from exists to defined 04/16/13 by dfouts
	    ...
	    print $FH "anno\t$fig\t$max_peak\t$figref->{$fig}->{'peak_number'}\n";
	}
	print $FH "$fig\t$counts\n";
...
$prog - Find prophage regions in DNA sequence from btab (blastp) and hmm data
 ...
 Option:
     ...
     -C: (C)onfig.ini file (default = phage_finder.ini)
     -E: (E)-value (default = 0.00001)
     -F: att site search (F)unction (B or b for NCBI BLASTN, M or m for MUMmer, F or f for FASTA) (default = BLASTN)
...
if (defined($hmmfile))  { # is there an hmm file?
    my $HMMFILE; # variable to hold the $hmmfile lexical filehandle
    open ($HMMFILE, '<:encoding(UTF-8)', $hmmfile) or  &write_log("4","can't open file HMM datafile $hmmfile: $!\n");
       while (<$HMMFILE>)  {
           if (/^# HMMER 3/) {
              $HMMversion = "3";
              last;
...
open ($APISFILE, '>:encoding(UTF-8)', $apisfile) or die "can not open APIS lookup file $apisfile for writing\n";
open ($TAB, '>:encoding(UTF-8)', $tabfile) or die "can not open file $tabfile for writing\n";
...
open ($SMALLTERMFILE, '>:encoding(UTF-8)', $smalltermpep) or die "can not open small terminase pepfile $smalltermpep for writing\n";
open ($LARGETERMFILE, '>:encoding(UTF-8)', $largetermpep) or die "can not open large terminase pepfile $largetermpep for writing\n";
open ($PORTALFILE, '>:encoding(UTF-8)', $portalpep) or die "can not open portal pepfile $portalpep for writing\n";
open ($PROTEASEFILE, '>:encoding(UTF-8)', $proteasepep) or die "can not open protease pepfile $proteasepep for writing\n";
open ($SCAFFOLDFILE, '>:encoding(UTF-8)', $scaffpep) or die "can not open scaffold pepfile $scaffpep for writing\n";
open ($CAPSIDFILE, '>:encoding(UTF-8)', $capsidpep) or die "can not open major capsid pepfile $capsidpep for writing\n";
open ($SERINERECOMBFILE, '>:encoding(UTF-8)', $serinerecombpep) or die "can not open serine recombinase pepfile $serinerecombpep for writing\n";
open ($TYROSINERECOMBFILE, '>:encoding(UTF-8)', $tyrosinerecombpep) or die "can not open tyrosine recombinase pepfile $tyrosinerecombpep for writing\n";
open ($TOXINSFILE, '>:encoding(UTF-8)', $toxinspep) or die "can not open toxins pepfile $toxinspep for writing\n";
open ($RBPSFILE, '>:encoding(UTF-8)', $rbpspep) or die "can not open toxins pepfile $rbpspep for writing\n";
open ($CONFILE, '>:encoding(UTF-8)', $confile) or die "can't open file $confile\n";
open ($SEQFILE, '>:encoding(UTF-8)', $seqfile) or die "can't open file $seqfile\n";
open ($PEPFILE, '>:encoding(UTF-8)', $pepfile) or die "can't open file $pepfile\n";
open ($BCNFILE, '>:encoding(UTF-8)', $badcomnamesfile) or &write_log("4","can't create file $badcomnamesfile: $!\n");
open ($FRAGFILE, '>:encoding(UTF-8)', $fragfile) or die "can't open file $fragfile\n"; # LinearDisplay.pl frag.file
open ($ATTFILE, '>:encoding(UTF-8)', $attfile) or die "can't open file $attfile\n"; # LinearDisplay.pl gene.att file
...
if ($genome_phage_cnt == "0") {
  my $TAB; # variable to hold the PFPR.tab lexical filehandle
  open ($TAB, '>:encoding(UTF-8)', $tabfile) or die "can not open file $tabfile for writing\n";
